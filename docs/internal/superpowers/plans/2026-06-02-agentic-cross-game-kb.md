...
| **KB-5c** | (If option 2) Implement `cli render <pack-root> --format=markdown --out=<file>` | Renders a deterministic markdown document from a pack's records, suitable for replacing `xedit-knowledgebase.md`. | Round-trip test: `cli render` then manual diff vs the previous handwritten file shows no critical content lost. |
