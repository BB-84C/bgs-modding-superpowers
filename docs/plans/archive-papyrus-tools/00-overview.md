...
- **bgs-archive**: round-trip REAL game archives. Open a real FO4/Starfield/Skyrim archive from the harness (`B:\WastelandBlues 2.0` or `.artifacts/mo2` real BA2s, or `.artifacts/bgs-mod-plugins/` samples), extract a known file, byte-compare against a reference extraction (BSArch.exe / B.A.E.), then repack and re-extract and confirm the payload round-trips. `ok:true` envelope alone is NOT acceptance — inspect extracted bytes.
