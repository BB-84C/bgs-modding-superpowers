...
`).run(record.id, packId, record.title, record.bodyMd, record.canonical.answer, JSON.stringify(record.appliesTo), jsonOrNull(record.variants), jsonOrNull(record.queryKeys), (record.queryKeys ?? []).join(" "), record.domains.join(" "), record.severity ?? null, record.canonical.confidence, JSON.stringify(record.sources), jsonOrNull(record.related), jsonOrNull(record.seeAlso), record.lastReviewed, record.schemaVersion);
        db.exec("COMMIT");
    }
    catch (error) {
        db.exec("ROLLBACK");
...
export function writePackMeta(db, meta, recordCount, builtAt) {
    const entries = {
        packId: meta.packId,
