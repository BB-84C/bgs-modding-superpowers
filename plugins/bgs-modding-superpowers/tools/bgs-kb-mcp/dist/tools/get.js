...
import { GameCodeEnum } from "../types/enums.js";
const Args = z
    .object({
    id: z.string().min(1),
...
export function makeGetTool(opts) {
    return async (rawArgs) => {
        ...
        const sources = baseRecord.sources ?? [];
        if (!game) {
            return ok({
                tool: "bgs_kb_get",
        ...
        const appliesTo = baseRecord.appliesTo;
        ...
        const variants = baseRecord.variants ?? {};
        ...
        try {
            ...
            return ok({
                ...
                data: { record: merged, mergedVariants: [game], appliesToRequestedGame: true, sources: merged.sources ?? sources },
                warnings,
                status: "completed",
            });
        }
...
function recordFromRow(row, session) {
    const sources = parseJsonColumn("sources_json", row.sources_json, []);
    return {
        id: row.id,
...
function mergeVariant(baseRecord, game, variant) {
    const merged = structuredClone(baseRecord);
    let body = String(merged.bodyMd ?? "");
    for (const deletion of variant.deletions ?? []) {
        if (!body.includes(deletion))
            throw new VariantDeletionUnmatched(game, deletion);
    ...
    if (additions.length > 0 || warnings.length > 0) {
        ...
        for (const warning of warnings)
            lines.push(`> [!WARNING] [${warning.code}|${warning.severity}] ${warning.text}`);
        body = `${lines.join("\n")}\n`;
    }
    merged.bodyMd = body;
    return merged;
}
function parseJsonColumn(columnName, rawValue, fallback) {
    if (!rawValue)
        return fallback;
    try {
