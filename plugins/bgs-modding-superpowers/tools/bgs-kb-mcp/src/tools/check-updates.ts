...
import { KB_ERROR_CODES, type Envelope, type Warning } from "../envelope/types.js";
...
export function makeCheckUpdatesTool(opts: CheckUpdatesToolOptions) {
  return async (rawArgs: Record<string, unknown>): Promise<Envelope<CheckUpdatesData>> => {
    ...
    try {
      ...
      const warning: Warning = { code: "release_index_fetch_failed", severity: "MEDIUM", message: `Release index fetch failed: ${message}` };
      return ok({
        tool: "bgs_kb_check_updates",
    ...
    const updates: CheckUpdateResult[] = [];
    for (const session of opts.registry.all()) {
      const pack = session.pack;
      if (requested && !requested.has(pack.packId)) continue;
