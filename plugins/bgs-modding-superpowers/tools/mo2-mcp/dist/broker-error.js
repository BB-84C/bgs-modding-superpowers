 ...
 */
import { probeMo2Process } from "./mo2-process-state.js";
 ...
 * original raw error.
 */
export function _classifyError(originalMessage, procState) {
    if (procState.alive && procState.responding === false) {
        return {
            code: "mo2_gui_unresponsive",
            message: "MO2 main thread is not responding — likely a modal dialog or long-running operation blocking the GUI",
 ...
 */
export async function enrichBrokerError(err, method, mo2Root, startedAt) {
    const originalMessage = err instanceof Error ? err.message : String(err);
    const procState = await probeMo2Process(mo2Root).catch(() => ({ alive: false }));
