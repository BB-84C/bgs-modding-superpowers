...
export async function fingerprintDir(path) {
    let fileCount = 0;
    let totalSize = 0;
...
export async function computeLease(targets) {
    const components = await Promise.all(targets.map((t) => t.kind === "text-file" ? fingerprintFile(t.path) : fingerprintDir(t.path)));
    const token = createHash("sha256")
        .update(JSON.stringify(components))
        .digest("hex");
    return { token, components };
}
export async function verifyLease(lease) {
    const drift = [];
