...
registerTool({
    ...
    handler: async (args, ctx) => {
        ...
        for (const f of files) {
            ...
            if (args.date_from && fileDate < args.date_from)
                continue;
            if (args.date_to && fileDate > args.date_to)
                continue;
            let text;
            try {
            ...
            for (const line of text.split("\n")) {
                ...
                if (args.tool && rec.tool !== args.tool)
                    continue;
                if (args.decision && rec.decision !== args.decision)
                    continue;
                if (args.plan_id && rec.plan_id !== args.plan_id)
                    continue;
                matched.push(rec);
