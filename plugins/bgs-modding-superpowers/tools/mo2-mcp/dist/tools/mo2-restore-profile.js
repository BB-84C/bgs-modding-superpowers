...
const handler = {
    ...
    async buildPlan(args, ctx) {
        ...
        return {
            ...
            targets: files.map((f) => ({ path: join(profileDir, f), kind: "text-file" })),
        };
    },
    async applyMutation(plan, ctx) {
        const profile = plan.args.profile ?? "Default";
        const label = plan.args.label;
        const backupDir = _backupDir(ctx, profile, label);
        const profileDir = resolveProfileDir(ctx, profile);
        ...
        await logApplyEvent(handler.toolName, `restored profile "${profile}" from label "${label}"`, requireBoundContext(ctx), plan.planId, profile);
...
registerTool({
    ...
    tier: "T3",
    description: "Restore profile state from a mo2_backup_profile label. Plan returns file count; apply copies all backed-up files back over profile.",
    inputSchema,
