...
function parseMetaIni(text: string): Record<string, MetaSection> {
  const sections: Record<string, MetaSection> = {};
  let current = "";
...
registerTool({
  ...
  tier: "T1",
  description:
    "Single mod detail: meta.ini parsed sections, file count, archive count (BA2/BSA), absolute path.",
  inputSchema,
  handler: async (args, ctx) => {
    ...
    let meta: Record<string, MetaSection> = {};
    try {
