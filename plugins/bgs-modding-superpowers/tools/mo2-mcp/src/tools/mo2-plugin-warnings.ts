...
registerTool({
  ...
  tier: "T1",
  description:
    "Poll MO2 for missing-master dependency warnings on enabled plugins. " +
  ...
  handler: async (args, ctx) => {
    ...
    const result = await pollPluginWarnings(bound.pipeClient, args.names as string[] | undefined);
    return {
      ok: true,
