...
async function _registerReinstalledPlugins(
  ctx: ToolContext,
  modPath: string,
): Promise<string[]> {
  const bound = requireBoundContext(ctx);
...
const handler: PlanApplyHandler = {
  ...
  async applyMutation(plan, ctx) {
    ...
    const buildConflictFields = async (): Promise<Record<string, unknown>> => {
      if (!bound.sidecar) return { conflicts_preview: CONFLICT_PREVIEW_SIDECAR_SKIPPED };
