...
async function _waitForMo2Gone(mo2Root: string): Promise<void> {
  for (let i = 0; i < 30; i++) {
    const det = await detectMo2Running({ mo2Root });
    if (!det.processRunning) return;
    await _delay(1000);
  }
...
registerTool({
  ...
  tier: "T3",
  description:
    "Cold-restart MO2 with a different profile. Shutdown → wait_dead → relaunch → wait_ready → sidecar_invalidate. Refuses profiles outside allowedProfiles.",
  inputSchema,
  handler: (args, ctx) =>
