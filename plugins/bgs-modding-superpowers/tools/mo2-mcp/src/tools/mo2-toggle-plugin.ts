...
const handler: PlanApplyHandler = {
  ...
  async buildPlan(args, ctx) {
    ...
    if (args.also_hide_file) {
      if (!bound.pipeClient) {
        throw new Error(
          "also_hide_file_requires_live_mo2: pipe needed to find owning mod via organizer.get_file_origins",
        );
      }
      const origin = await bound.pipeClient.call("organizer.get_file_origins", {
        filename: args.name,
      });
      const owners = (origin.result as { origins?: string[] } | undefined)?.origins ?? [];
      ...
      const espPath = join(modsDir, ownerMod, args.name as string);
    ...
    return {
      diff: `plugins.txt: ${args.enabled ? "*" : ""}${args.name}${
        args.also_hide_file ? ` + file ${args.enabled ? "unhide" : "→ .mohidden"}` : ""
      }`,
  ...
  async applyMutation(plan, ctx) {
    ...
    let brokerResult: Record<string, unknown> | undefined;
    if (bound.pipeClient) {
      await assertActiveProfile(ctx, profile);
      ...
      brokerResult = resp.result as Record<string, unknown> | undefined;
    }
    ...
    const lines = text.split(/\r?\n/).map((l) => {
      ...
      if (bare === args.name) return (args.enabled ? "*" : "") + (args.name as string);
      return l;
    });
...
registerTool({
  ...
  tier: "T3",
  description:
    "Enable/disable plugin. Optional also_hide_file=true renames the .esp to .mohidden for Optional-ESP semantics (live-mode only).",
  inputSchema,
  handler: (args, ctx) =>
