...
const LAUNCH_OVERRIDE_PROPERTIES = {
    ...
    dataPath: {
        type: "string",
        description: "-D: flag value: absolute path to the game Data directory. Pass MO2's <gamePath>\\Data to avoid xEdit's registry-discovered Steam path. Use backslashes; the launcher normalizes mixed slashes.",
    },
    pluginsFile: {
        type: "string",
...
export const TOOL_DEFINITIONS = [
    ...
    {
        name: "xedit_dirty",
        description: "Returns xEdit's dirty state immediately. When ready: wraps session.get_dirty_state and returns { dirty, dirtyFiles, unsavedChangeCount }. Otherwise: returns the same shape as xedit_status.",
        inputSchema: { type: "object", properties: {}, additionalProperties: false },
    },
    {
        name: "xedit_stop",
        description: "Stops the xEdit daemon and clears MCP state. Before shutdown it checks session.get_dirty_state. If there are unsaved changes and force!==true, returns code='dirty_state' with dirtyFiles instead of stopping. If the daemon is a zombie, use force:true to abandon unsaved work and clear the state.",
        inputSchema: {
            type: "object",
            properties: {
                force: {
                    type: "boolean",
                    description: "If true, stop even when xEdit has unsaved changes (abandons in-memory edits). Default false.",
                },
            },
            additionalProperties: false,
        },
    ...
    {
        name: "xedit_restart",
        description: "Stops the current daemon (same dirty-state safety as xedit_stop) and immediately kicks off a fresh asynchronous launch. Accepts the same overrides as xedit_start plus force?: boolean. Use this to reboot with a new pluginsFile or dataPath instead of reconnecting /mcp manually.",
        inputSchema: {
            type: "object",
            properties: {
                ...
                force: {
                    type: "boolean",
                    description: "If true, restart even when xEdit has unsaved changes (abandons in-memory edits). Default false.",
                },
            },
            additionalProperties: false,
        },
    ...
    },
    {
        ...
        description: "Requires the daemon to be ready. Locates records by either {file, formId} (exact override lookup) OR {editorId, signature?} (Editor ID search across the load order). " +
        ...
        inputSchema: {
            ...
            properties: {
                file: {
                    type: "string",
                    minLength: 1,
                    description: "Plugin filename including extension, e.g. 'kinggathcreations_spaceship.esm'. Required for the {file, formId} search mode. Do NOT pass an empty string in editorId mode.",
                },
                formId: {
                    type: "string",
                    pattern: FORM_ID_PATTERN,
                    description: "FormID as hex, with or without 0x prefix, e.g. '0000003C' or '0x0000003C'. Up to 8 hex digits. Required for the {file, formId} search mode. Do NOT pass a zero placeholder in editorId mode.",
                },
                editorId: {
                    type: "string",
                    minLength: 1,
                    description: "Editor ID to search for, e.g. 'PlayerRef'. Required for the {editorId} search mode.",
                },
                signature: {
                    type: "string",
