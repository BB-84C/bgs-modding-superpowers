...
| `xedit-wsl-bridge/` | PowerShell HTTP→NamedPipe proxy so WSL agents can reach the xEdit daemon. Ships `bridge.ps1` (long-running proxy on port 42423), `query.ps1` (one-shot pipe helper), and `find_pipe.ps1` (diagnostic). | New |
