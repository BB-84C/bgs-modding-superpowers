...
export async function pruneCache(cacheRoot, opts = {}) {
    ...
    for (const packId of packIds) {
        ...
        if (!dryRun) {
            for (const version of removed)
                await rm(join(cacheRoot, packId, version), { recursive: true, force: true });
        }
        packs.push({ packId, kept, removed });
    }
    return { cacheRoot, dryRun, packs };
}
export function formatPruneCacheResult(result) {
    if (result.packs.length === 0)
        return `no packs cached at ${result.cacheRoot}\n`;
    return result.packs
        .map((pack) => {
...
//# sourceMappingURL=prune-cache.js.map