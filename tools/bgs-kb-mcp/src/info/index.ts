...
import type { PackManifest, SourceRecord } from "../build/types.js";

const require = createRequire(import.meta.url);
const { DatabaseSync } = require("node:sqlite") as {
  DatabaseSync: new (path: string, options?: { readOnly?: boolean }) => InfoDbHandle;
};

interface InfoDbHandle {
...
function sortEntries(record: Record<string, number>): Record<string, number> {
  return Object.fromEntries(Object.entries(record).sort(([a], [b]) => a.localeCompare(b)));
}

function countRecords(records: SourceRecord[], values: (record: SourceRecord) => string[]): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const record of records) {
    for (const value of values(record)) counts[value] = (counts[value] ?? 0) + 1;
  }
  return sortEntries(counts);
}

function uniqueSorted(records: SourceRecord[], values: (record: SourceRecord) => string[]): string[] {
...
function countRows(rows: unknown[], key: string): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const row of rows) {
    const obj = row as Record<string, unknown>;
    const name = obj[key];
    const count = obj.n;
    if (typeof name === "string" && typeof count === "number") counts[name] = count;
  }
  return sortEntries(counts);
}

async function readManifest(manifestPath: string): Promise<PackManifest | undefined> {
  if (!existsSync(manifestPath)) return undefined;
  return JSON.parse(await readFile(manifestPath, "utf8")) as PackManifest;
}

function readSqlite(sqlitePath: string): { recordCount: number; byDomain: Record<string, number>; byGame: Record<string, number> } {
...
export async function gatherInfo(packRoot: string): Promise<PackInfo> {
  ...
  let sqlite: SqliteInfo | undefined;
  let liveByDomain: Record<string, number> | undefined;
  let liveByGame: Record<string, number> | undefined;
  if (!existsSync(sqlitePath)) {
    ...
    if (sha256Verified === false) warnings.push("kb.sqlite sha256 does NOT match manifest. Rebuild recommended.");
...
export function fallbackPackId(info: PackInfo): string {
