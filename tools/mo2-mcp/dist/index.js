 ...
 */
export function normalizeMcpInputSchema(schema) {
    const fallback = {
        type: "object",
        properties: {},
    ...
    if (!schema || typeof schema !== "object")
        return fallback;
    const unionKeywords = ["anyOf", "oneOf", "allOf"];
    ...
    if (topProps && typeof topProps === "object") {
        for (const [k, val] of Object.entries(topProps)) {
            if (!(k in mergedProperties))
                mergedProperties[k] = val;
        }
    }
    for (const branch of branches) {
        ...
        if (bp && typeof bp === "object") {
            for (const [k, val] of Object.entries(bp)) {
                if (!(k in mergedProperties))
                    mergedProperties[k] = val;
            }
        }
...
function extractDiscriminants(branches) {
    ...
    for (const branch of branches) {
        ...
        const props = b.properties && typeof b.properties === "object"
            ? b.properties
            : {};
        const req = Array.isArray(b.required) ? b.required : [];
        const map = new Map();
