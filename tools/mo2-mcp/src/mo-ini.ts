 ...
 */
export function decodeIniValue(value: string): string {
...
const GAME_KEY_TO_NAME: Record<string, string> = (() => {
  const m: Record<string, string> = {};
  for (const [name, key] of Object.entries(GAME_NAME_TO_KEY)) {
    if (!(key in m)) m[key] = name; // first-wins for shared keys (Fallout4VR→fallout4 etc.)
  }
  return m;
})();
 ...
 */
export function resolveGameKey(general: { game?: string; gameName?: string }): string {
  if (general.game) return general.game;
  if (general.gameName) {
 ...
 */
export function resolveGameName(general: { game?: string; gameName?: string }): string {
  if (general.gameName) return general.gameName;
  if (general.game && GAME_KEY_TO_NAME[general.game]) return GAME_KEY_TO_NAME[general.game];
  return "Fallout4";
}
