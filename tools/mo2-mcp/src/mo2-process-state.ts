...
export async function probeMo2Process(mo2Root: string): Promise<Mo2ProcessState> {
  try {
    ...
    const parsed = JSON.parse(trimmed) as
      | { Id?: number; Responding?: boolean; StartTime?: string | null }
      | Array<{ Id?: number; Responding?: boolean; StartTime?: string | null }>;
    const proc = Array.isArray(parsed) ? parsed[0] : parsed;
    if (!proc || typeof proc.Id !== "number") return { alive: false };
    return {
      alive: true,
      pid: proc.Id,
