 ...
 */
function mapParentSpecForDaemon(args) {
    const parent = args.parent;
    if (!parent || typeof parent !== "object")
        return args;
    const p = parent;
    const daemonParent = {};
    if (typeof p.parentFile === "string")
        daemonParent.file = p.parentFile;
    if (typeof p.parentFormId === "string") {
        const f = p.parentFormId;
        daemonParent.formId = f.startsWith("0x") || f.startsWith("0X") ? f.slice(2) : f;
    }
    if (typeof p.subGroup === "string")
        daemonParent.subGroup = p.subGroup;
    if (Array.isArray(p.coords))
        daemonParent.coords = p.coords;
